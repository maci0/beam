#!/usr/bin/env bash
# 2-node ROCm tensor-parallel inference: beam head on this host, worker on a
# second AMD box, beam as the ray backend. Consumer cards have no RoCE, so RCCL
# runs over TCP sockets (NCCL_IB_DISABLE=1). If the two cards are different archs
...
WORK_IP="${WORK_IP:-10.0.0.2}"                  # second AMD node
HEAD_IF="${HEAD_IF:-eth0}"                       # interface carrying HEAD_IP here
WORK_IF="${WORK_IF:-eth0}"                       # interface carrying WORK_IP there
SSH_USER="${SSH_USER:-$USER}"
HEAD_DIR="$(cd "$(dirname "$0")/.." && pwd)"     # repo on this host
WORK_DIR="${WORK_DIR:-$HOME/beam}"               # beam deployed on the worker
IMAGE="${IMAGE:-rocm/vllm-dev:nightly}"
RENDER="${RENDER:-/dev/dri/renderD128}"          # discrete GPU render node (verify per host)
...
cleanup() {
  ...
  $SSH "$SSH_USER@$WORK_IP" 'bash -lc "docker rm -f beam-rw 2>/dev/null"' >/dev/null 2>&1 || true
}
trap cleanup EXIT
cleanup
...
$SSH "$SSH_USER@$WORK_IP" "bash -lc 'docker rm -f beam-rw 2>/dev/null; docker run -d --name beam-rw $common \
